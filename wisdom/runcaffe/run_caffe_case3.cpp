...
vector<DETECT_BOX_S> run_caffe_once_case3(cv::Mat& org_img, cv::Rect& padrect, MODEL_INFO_S *pstInfo)
{
...
vector<DETECT_BOX_S> run_caffe_pad_case3(cv::Mat& org_img, MODEL_INFO_S *pstInfo)
{
    ...
    Net<float> *caffe_net = pstInfo->caffe_net;
    vector<DETECT_BOX_S> retbox;
    std::cout << "run_caffe_pad_case3" << endl;
    ...
    retbox = run_caffe_once_case3(org_img, padrect, pstInfo);
    return retbox;
}

vector<DETECT_BOX_S> run_caffe_other_case3(cv::Mat& org_img, MODEL_INFO_S *pstInfo)
{
    Blob<float>* input_layer = pstInfo->caffe_net->input_blobs()[0];
    int width = input_layer->width();
    int height = input_layer->height();
    Net<float> *caffe_net = pstInfo->caffe_net;
    vector<DETECT_BOX_S> retbox;
    std::cout << "run_caffe_other_case3" << endl;
    cv::Mat img = Preprocess(org_img);
    int srch = img.rows;
    int srcw = img.cols;
...
vector<DETECT_BOX_S> run_caffe_case3(cv::Mat& org_img, MODEL_INFO_S *pstInfo)
{
    vector<DETECT_BOX_S> retbox;
    Blob<float>* input_layer = pstInfo->caffe_net->input_blobs()[0];
    input_layer->Reshape(1, input_layer->shape(1),
                       (org_img.rows+127)&(~127), 
                       (org_img.cols+127)&(~127));
    ...
    {
        retbox = run_caffe_pad_case3(org_img, pstInfo);
    }
    else
    {
        retbox = run_caffe_other_case3(org_img, pstInfo);
    }
